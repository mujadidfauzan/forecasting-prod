...
model_bilstm = tf.keras.models.load_model("my_model-bilstm-window_4.keras")
...
def getData(step):
    """
    Retrieve recent Prometheus data for SIEM, and return a seed for forecasting.
    This function extracts the last 4 data points (to match the training window)
    and scales them using the same scaler as during training.
    """
    ...
    query = '(node_memory_MemTotal_bytes{job="SIEM"} - node_memory_MemAvailable_bytes{job="SIEM"}) * 100 / node_memory_MemTotal_bytes{job="SIEM"}'
